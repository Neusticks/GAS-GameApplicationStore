...
class _EditProductFormState extends State<EditProductForm> {
  ...
  void initState() {
    ...
    if (widget.product == null) {
      ...
      productDetails.initialSelectedImages = widget.product.productImages
          .map((e) => CustomImage(imgType: ImageType.network, path: e))
          .toList();
      productDetails.initialProductType = product.productType;
      productDetails.initSearchTags = product.productSearchTags ?? [];
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (newProduct == false) {
      titleFieldController.text = product.productName;
      variantFieldController.text = product.productVariant;
      discountPriceFieldController.text = product.productDiscountPrice.toString();
      originalPriceFieldController.text = product.productOriginalPrice.toString();
      highlightsFieldController.text = product.productHighlights;
      desciptionFieldController.text = product.productDescription;
      sellerFieldController.text = product.sellerId;
    }
    return column;
  }
    ...
    return Consumer<ProductDetails>(
      builder: (context, productDetails, child) {
        return Tags(
          ...
          heightHorizontalScroll: getProportionScreenHeight(80),
  ...
  bool validateBasicDetailsForm() {
    if (_basicDetailsFormKey.currentState.validate()) {
      ...
      product.productName = titleFieldController.text;
      product.productVariant = variantFieldController.text;
      product.productOriginalPrice = double.parse(originalPriceFieldController.text);
      product.productDiscountPrice = double.parse(discountPriceFieldController.text);
      product.sellerId = sellerFieldController.text;
      return true;
    }
    return false;
  }
  ...
  bool validateDescribeProductForm() {
    if (_describeProductFormKey.currentState.validate()) {
      ...
      product.productHighlights = highlightsFieldController.text;
      product.productDescription = desciptionFieldController.text;
      return true;
    }
    return false;
  }
  ...
  Future<void> saveProductButtonCallback(BuildContext context) async {
    ...
    try {
      ...
      product.productSearchTags = productDetails.searchTags;
